...
local constHiResThreshold  	= 1.8   -- Hi-res pic will be loaded if the app is "magnified" by this or greater value;
									-- if width in config.lua is 320, then hi-res is loaded starting from device width = 320 * 1.8 = 576 
...
---------------------------------------------------------------
local function loadImage(imageFileName, forceLoadRes)	-- The main method; optional parm forceLoadRes = "high" / "low" or empty
---------------------------------------------------------------
...
if forceLoadRes ~= "high" and forceLoadRes ~= "low" then forceLoadRes = "" end
...
------------------------------------------------------
-- screen height in "application pixels"
-- in "letterbox" scaling mode works only with yAlign = "bottom" or "center" (default); 
-- doesn't work for yAlign = "top" (we can't get the height of the bottom margin)
...
if yAlign ~= "bottom" then yAlign = "center" end
	...
	if scaleMode == "none" or scaleMode == "letterbox" then
		if yAlign == "center" then
			return display.viewableContentHeight - 2 * display.screenOriginY
		else
			return display.viewableContentHeight - display.screenOriginY
		end
	else
...
------------------------------------------------------
-- screen width in "application pixels"
-- in "letterbox" scaling mode works only with xAlign = "right" or "center" (default); 
-- doesn't work for xAlign = "left" (we can't get the width of the right margin)
...
if xAlign ~= "right" then xAlign = "center" end
	...
	if scaleMode == "none" or scaleMode == "letterbox" then
		if xAlign == "center" then
			return display.viewableContentWidth - 2 * display.screenOriginX
		else
			return display.viewableContentWidth - display.screenOriginX
		end
	else
